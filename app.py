...
def index():
    ...
    try:
        ...
        if plot_bool == False:
            ...
            fig = tpl.figure()
            fig.barh([int(value) for value in values], list(keys), force_ascii=True)
            fig_string = fig.get_string()
            json_object = {"fig": fig_string}
            response = app.response_class(response=json.dumps(json_object), status=200, mimetype="application/json")
...
def get_image():
    ...
    response = app.response_class(response=json.dumps(img), status=200, mimetype="application/json")
...
def translate():
    ...
    response = app.response_class(response=r.content, status=200, mimetype="application/json")
